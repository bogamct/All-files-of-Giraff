...
#define WHEELBASE      0.47     // in meters. measured 0.456, adding fudge factor to account for actual experimentally correct value 
#define WHEEL_DIAMETER 0.145    // in meters
// no longer used #define KI_DEFAULT 50

// 17 is the gear ratio
...
#define WHEEL_CLICKS_PER_OUTPUT_REVOLUTION  (17 * 250.0 * 2)
...
// reworked servo code to use radians instead of arbitrary units - 5/19/09
...
// defining everything in terms of degrees for human-readability

// define encoder counts per radian.
// assume 62:1 gear ratio and 360 count/rev encoder. That means 62*360 = 22320 counts/rotation
#define SERVO_TICKS_PER_RADIAN (22320.0*2 / (2*PI))
...
#define SERVO_HOMING_VELOCITY (DEFAULT_MAX_SERVO_VELOCITY / 10)


//Variables below are different between hardware versions and are moved from the autogen_variables

//tilt_angle_from_home
#define TILT_ANGLE_FROM_HOME_VAR_MIN 5*(3.14159/180.0)
#define TILT_ANGLE_FROM_HOME_VAR_MAX 120*(3.14159/180.0)
#define TILT_ANGLE_FROM_HOME_VAR_DEFAULT TILT_ANGLE_FROM_HOME_VAR_MIN

/*-----Height adjustment variables------*/
//Currently not in use on this HW version
