...
void a2dInit() {
  ...
  PORTA &= ~ (_BV(PA0) | _BV(PA1) | _BV(PA2) | _BV(PA3) | _BV(PA4) | _BV(PA5) | _BV(PA6));

  ADCSRA = (1<<ADEN) | (1<<ADPS2) | (1<<ADPS1); //ADC enabled and clock prescaler=64
  ADMUX = (1<<REFS1) | (1<<REFS0); //INternal 2.56V reference 
  DIDR0 = (1<<ADC7D) | (1<<ADC6D) | (1<<ADC5D) | (1<<ADC4D) | (1<<ADC3D) | (1<<ADC2D) | (1<<ADC1D) | (1<<ADC0D); //Digital input disabled, this is to save power
#endif
}
...
void a2dChooseChannel( u08 p_adcCommand ) {
  ...
  case CMD_DOCK_CURRENT:
    ADMUX=(ADMUX & (128+64+32)) | ((1<<MUX3) | (1<<MUX2) | (1<<MUX0)); //ADC3-ADC2 10x
    break;
  case CMD_DOCK_VOLTAGE:
    ADMUX=(ADMUX & (128+64+32)) | (1<<MUX2); //ADC4
    break;
  case CMD_BATT_CURRENT:
    ADMUX=(ADMUX & (128+64+32)) | ((1<<MUX3) | (1<<MUX0)); //ADC1-ADC0 10x
    break;
  case CMD_BATT_VOLTAGE:
    ADMUX=(ADMUX & (128+64+32)) | ((1<<MUX2) | (1<<MUX0)); //ADC5
    break;
  case CMD_BATT_TEMPERATURE:
    ADMUX=(ADMUX & (128+64+32)) | ((1<<MUX2) | (1<<MUX1)); //ADC6
    break;
  case CMD_OFFSET_ADJUSTMENT:
    ADMUX=(ADMUX & (128+64+32)) | (1<<MUX4); //ADC0-ADC0 10x (Offset adjustment)
    break;
  default:
