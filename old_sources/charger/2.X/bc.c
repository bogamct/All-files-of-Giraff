...
#define ONOFF_BUTTON_DEBOUNCE_MICROS 2000
...
#define ONOFF_BUTTON_DEBOUNCE_TICKS (( MICROS_TO_TIMER_TICKS( ONOFF_BUTTON_DEBOUNCE_MICROS ) ))

/*#define MB_STATE_OFF 0
#define MB_STATE_ON 1
#define MB_STATE_TURNING_OFF 2
#define MB_STATE_TURNING_ON 3*/

#define NUM_MESSAGES 9

volatile u16 m_giraffOnOffButtonDebounceTimer = 0;
...
int main() {
  motherboardInit(); //Also includes init of on/off button
  rtcInit();
  ledsInit();
  ...
  while (true) {
    // if we're in an error state, displayStatus does nothing with the internal LEDs. The error gets display in the
    // error() method.  Otherwise, we set the two LEDs to show whether the system is docked, fast charging, ...
    // Only need to update status LEDs every 0.2s
    if ( getSetTimer( &m_ledTimer, LED_TIMER_TICKS ) ) {
      displayStatus(m_wattSeconds);
    } 
    doImportantStuff();

#ifdef SIMULATE
    //Only execute the tests every second clock tick since it takes two laps to switch states some times
	if (m_rtcTimer % 2 == 0 ) {
		//Send the milliseconds value since we can't access it in the executor otherwise
		testCaseExecutor( (RTC_TIMER_TICKS-1)-m_rtcTimer );
	}
#endif
    outputChargerData();
	...
	if ( m_giraffOnOffButtonDebounceTimer == 0 ) {
...
void init() {
  ...
  //This is used during startup to test all diodes. 
  //All doides should blink during startup
  //Note that the battery should be yellow since both red and green is tested
  if ( getSetTimer( &m_ledTimer, LED_TIMER_TICKS ) ) {
    setLED( INTERNAL_RED, !getLED( INTERNAL_RED ) );
    setLED( INTERNAL_GREEN, !getLED( INTERNAL_GREEN ) );
    setLED( ONOFF_LED, !getLED( ONOFF_LED ) );
    setLED( CHARGING_LED , !getLED( CHARGING_LED  ) );
    setLED( BAT_LVL_RED, !getLED( BAT_LVL_RED ) );
    setLED( BAT_LVL_GREEN, !getLED( BAT_LVL_GREEN ) );
  }
...
#ifndef SIMULATE
//Giraff ON/OFF button interrupt
SIGNAL(SIG_INTERRUPT0) {
  if( isGiraffOnOffButtonPressed() ) { 
    if ( isMotherboardOn() ) {
      setMotherboardOnOffIndicatorInterrupt(true);
    } else { 
      setMainPower(true);
    }
    setMotherboardOnOfSwitchPin(true);
  } else {
    setMotherboardOnOfSwitchPin(false);
  }
  
  //Make sure that no bouncing occurs during this period of time
  setGiraffOnOffButtonInterrupt(false);
  m_giraffOnOffButtonDebounceTimer = ONOFF_BUTTON_DEBOUNCE_TICKS;
}

//MB ON/OFF indicator interrupt, occurs when motherboard turns off
